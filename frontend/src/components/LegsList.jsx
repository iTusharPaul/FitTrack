...
export default function LegsList() {
  const [LegsWorkouts, setLegsWorkouts] = useState(null);
  const [selectedWorkout, setSelectedWorkout] = useState(null);

  useEffect(() => {
    const fetchExercises = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/user/exercises/Legs`, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
        });
        setLegsWorkouts(response.data);
      } catch (e) {
        console.log("Legs exercises could not be fetched " + e.message);
      }
    };
  ...
  return (
    <>
      ...
      >
        ...
        >
          Time to lift heavy—Log your legs workout now!
        </Typography>
      </div>
    </>
  );
