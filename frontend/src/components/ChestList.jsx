...
export default function ChestList() {
  const [ChestWorkouts, setChestWorkouts] = useState(null);
  const [selectedWorkout, setSelectedWorkout] = useState(null);

  useEffect(() => {
    const fetchExercises = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/user/exercises/Chest`, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
        });
        setChestWorkouts(response.data);
      } catch (e) {
        console.log("Chest exercises could not be fetched " + e.message);
      }
    };
  ...
  return (
    <>
      ...
      >
        ...
        >
          Unleash your strength—Log your chest workout now!
        </Typography>
      </div>
    </>
  );
